...
fig = plt.figure(figsize = (10,10))
for ii in range(len(doubling_times)):
    k = np.log(2)/doubling_times[ii]
    t = np.linspace(0, 30, 30)
    plt.plot(t, growthcurve(1, t, k, kl, LC50, Sm, c, SC50, h), linewidth=5, label=str(doubling_times[ii])+'h')
    for i in [0, 4, 8, 12, 16, 20, 24]:
        plt.plot(i, growthcurve(1, i, k, kl, LC50, Sm, c, SC50, h), 'o', markersize=9, color='black')
# for j in range(1,len(x0_vect)):
#     time_adj = np.arange(toD[j], tf, step=1)
#     plt.plot(time_adj[0], x0_vect[j], 'o', markersize=9, label=str(toD[j])+'h')
plt.xlabel('Time')
plt.ylabel('Normalized cell count')
plt.legend(loc='best')
plt.savefig(path+'Cell_count_diff_doubling_times.svg')
plt.show()
